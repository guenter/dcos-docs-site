...
If the scheduler is running but no other tasks are being launched and the DC/OS cluster is running in strict mode, verify that the service account was [created correctly](/services/kubernetes/__VERSION__/operations/customizing-install/#tls).
...
In this case use the the `dcos kubernetes cluster debug pod` [CLI](/services/kubernetes/__VERSION__/cli#debug-pod-1) to assist in debugging and recovering failed, flapping or otherwise misbehaving tasks.
