...
See the [Resources section](../getting-started/install-basic/#resources) of the [Installing](../getting-started/install-basic/) page for more detail on setting up your cluster.

## Scheduler is restarting

If the scheduler task is starting for your Kubernetes cluster but is in a "Failed" restart loop, verify all of the installation [prerequisites](../getting-started/install-basic/#prerequisites) have been met.
...
If the scheduler is running but no other tasks are being launched and the DC/OS cluster is running in strict mode, verify that the service account was [created correctly](../operations/customizing-install/#tls).
...
Before re-installing DC/OS Kubernetes, make sure that all DC/OS agents are healthy, and that the DC/OS cluster itself is healthy and that there are no networking problems, and meets all the [prerequisites](../getting-started/install-basic/#prerequisites).
...
When the DC/OS agent where `etcd-0-peer` was running permanently fails, and as mentioned in [Limitations](../limitations/), the contents of the `etcd` data directory will be **PERMANENTLY LOST**.
